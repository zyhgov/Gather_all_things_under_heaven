document.addEventListener('DOMContentLoaded', () => {
  ...
  if (username && useremail) { // 确保两个变量都存在
      ...
      fileInput.addEventListener('change', async () => {
          ...
          // 调用带有进度监控的上传函数
          await uploadWithProgress(file, userId, supabaseUrl, supabaseKey);
      });
      // 添加安全退出事件监听器
...
async function uploadWithProgress(file, userId, supabaseUrl, supabaseKey) {
  ...
  xhr.upload.onprogress = function(event) {
      if (event.lengthComputable) {
          ...
          uploadMessage.textContent = `上传进度: ${Math.round(percentComplete)}%`;
      }
  };
